...
- **Where it is today** — all single-cycle datapath and control modules are written and verified individually — ALU, register file, immediate generator, decoder, PC unit, instruction and data memory; the ALU has additionally been synthesised and tested on real hardware. Wiring them into one top level is in progress. Pipelining has not started yet.
...
| `pc_unit.v` — PC register, PC+4 and branch-target adders, next-address select (12/12 checks pass) | | |
